...
ray_date_starts = [dt.datetime(2020,7,25,0,5,0,tzinfo=dt.timezone.utc)]
ray_date_start = dt.datetime(2020,7,25,0,5,0,tzinfo=dt.timezone.utc)
wprs = np.array([10])
nray_use = 50000
wprs = wprs/nray_use
...
if run_the_rays == True:
    for md in mds:
        for tti in tt_increment:
            ...
            directions, ra, thetas, phis = getBdir(ray_start, ray_datenum, rayfile_directory, thetas, phis, md, freq, select_random=True)
    ...
    for ray_date_start, freq, wpr, opp_hemi in zip(ray_date_starts, freqs, wprs, opp_hemis):
        for md in mds:
            for tti in tt_increment:
                ...
                if freq > 15e3:
                    nmax = plotray2D(ray_datenum, raylist, ray_out_dir, 'GEO', 'car', ['Re','Re','Re'], md, checklat, plot_wna=False, show_plot=False)
                else:
                    nmax = plotray2D(ray_datenum, raylist, ray_out_dir, 'GEO', 'car', ['Re','Re','Re'], md, checklat, plot_wna=True, show_plot=False)
